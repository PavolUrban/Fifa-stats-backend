...
public interface MatchesRepository extends CrudRepository<Matches, Long>{
	...
	@Query("SELECT m FROM Matches m " +
			"where (:season is null or m.season = :season) and (:competition is null or m.competition = :competition) and (:competitionPhase is null or m.competitionPhase = :competitionPhase) and  ( (:teamId is null or m.homeTeam.id = :teamId) or (:teamId is null or m.awayTeam.id = :teamId))" +
	...
	@Query(
			"SELECT m " +
			"FROM Matches m " +
			"WHERE m.season = ?1 AND m.competition= ?2 AND m.competitionPhase IN ('Round of 32','Round of 16','Quarterfinals', 'Semifinals', 'Final') ")
	...
	@Query(
			"SELECT m " +
			"FROM Matches m " +
			"WHERE m.season = ?1 AND m.competition= ?2 AND m.competitionPhase IN ?3 ")
	List<Matches> getAllMatchesBySeasonCompetitionAndCompetitionPhaseIn(String season, String competition, List<String> competitionPhase);

	@Query(value =
			"SELECT * FROM Matches m " +
			"WHERE (:competition is null or m.competition = :competition) AND ( (:teamId is null or m.idhometeam = :teamId) OR (:teamId is null or m.idawayteam = :teamId) ) " +
			"ORDER BY (m.scorehome + m.scoreaway) DESC, m.scorehome DESC, m.scoreaway DESC,  m.season LIMIT 100", nativeQuery = true)
	List<Matches> getMatchesWithMostGoals(String competition, Long teamId);

	@Query(value =
			"SELECT * FROM Matches m  " +
			"WHERE (:playerName is null or m.playerA = :playerName) AND (:competition is null or m.competition = :competition) AND (:teamId is null or m.winnerid = :teamId)  AND (:teamId is null or m.idawayteam = :teamId) " +
			"ORDER BY (m.scoreaway - m.scorehome) DESC, m.scoreaway DESC, m.scorehome DESC,  m.season LIMIT 100", nativeQuery = true)
	List<Matches> getBiggestAwayWins(String playerName, String competition, Long teamId);

	@Query(value =
			"SELECT * FROM Matches m " +
			"WHERE (:playerName is null or m.playerH = :playerName) AND (:competition is null or m.competition = :competition) AND (:teamId is null or m.winnerid = :teamId) AND (:teamId is null or m.idhometeam = :teamId) " +
			"ORDER BY (m.scorehome - m.scoreaway) DESC, m.scorehome DESC, m.scoreaway DESC,  m.season LIMIT 100", nativeQuery = true)
	...
	@Query(value =
			"SELECT * FROM Matches m " +
			"WHERE (:competition is null or m.competition = :competition) AND ( (:teamId is null or m.idhometeam = :teamId) OR (:teamId is null or m.idawayteam = :teamId) ) " +
			"HAVING (m.scoreaway-m.scorehome) = 0 " +
			"ORDER BY m.scorehome DESC LIMIT 100", nativeQuery = true)
